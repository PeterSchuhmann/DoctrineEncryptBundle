...
namespace Resomedia\DoctrineEncryptBundle\Subscribers;
...
use Resomedia\DoctrineEncryptBundle\Encryptors\EncryptorInterface;

/**
 * Source : Ambta\DoctrineEncryptBundle (modified)
...
class DoctrineEncryptSubscriber implements EventSubscriber {
     ...
     */
    public function __construct(Reader $annReader, $encryptorClass, $secretKey, $protocol, $iv, EncryptorInterface $service = NULL) {
        ...
        if ($service instanceof EncryptorInterface) {
            ...
            $this->encryptor = $this->encryptorFactory($encryptorClass, $secretKey, $protocol, $iv);
        }
    ...
    public function setEncryptor($encryptorClass) {
        if(!is_null($encryptorClass)) {
            $this->encryptor = $this->encryptorFactory($encryptorClass, $this->secretKey, $this->protocol, $this->iv);
            return;
        }
    }
     ...
     * @param string $protocol Cipher
     * @param string $iv
     *
     * @return EncryptorInterface
     * @throws \RuntimeException
     */
    private function encryptorFactory($classFullName, $secretKey, $protocol, $iv) {
        $refClass = new \ReflectionClass($classFullName);
        if ($refClass->implementsInterface(EncryptorInterface::class)) {
            return new $classFullName($secretKey, $protocol, $iv);
        } else {
